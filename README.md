# Data Structures and Algorithms

## My Implementations

- [Linked List using a Hash Table](implementations_of_data_structures/LinkedList.js)
- [Stack using a Linked List](implementations_of_data_structures/Stack.js)
- [Stack using a Hash Table](implementations_of_data_structures/Stack2.js)
- [Queue using a Hash Table](implementations_of_data_structures/Queue.js)
- [Hash Table using an Array](implementations_of_data_structures/HashTable.js)
- [K Stacks using One Array](arrays/KStacks.js)

## Prompts and Solutions

Prompts and Solutions were provided by [Bianca Gandolfo](https://twitter.com/biancagando) and [Frontend Masters](https://frontendmasters.com/).

### [Introduction to Data Structures](https://frontendmasters.com/courses/data-structures-interviews/)

#### Data Structures - Prompts and Solutions

- [Stack](https://replit.com/@bgando/stack-prompt)
  - [Stack Solution](https://replit.com/@bgando/stack-solution-1)
- [Queue](https://replit.com/@bgando/queue-prompt)
  - [Queue Solution](https://replit.com/@bgando/queue-solution-1)
- [Linked List](https://replit.com/@bgando/linked-list-prompt)
  - [Linked List Solution](https://replit.com/@bgando/linked-list-solution)
- [Hash Table](https://replit.com/@bgando/hash-table-prompt)
  - [Hash Table Solution](https://replit.com/@bgando/hash-table-solution-1)
- [Array](https://replit.com/@bgando/array-prompt)

#### Stacks/Queues - Prompts and Solutions

- [Use an array to implement 3 stacks](https://repl.it/@bgando/k-stacks-one-array-prompt)
  - [Solution](https://replit.com/@bgando/k-stacks-one-array-solution)
- [Implement a getMin() or getMax() method on your stack](https://repl.it/@bgando/min-stack-prompt)
  - [Solution](https://replit.com/@bgando/min-stack-prompt)
- [Create a queue using 2 stacks](https://repl.it/@bgando/queue-two-stacks-prompt)
  - [Solution](https://replit.com/@bgando/queue-two-stacks-solution)
- [Sort a stack with the min values in order, on top. You can use another stack as a buffer.](https://repl.it/@bgando/sort-stack-prompt)
  - [Solution](https://replit.com/@bgando/sort-stack-solution)
- [Write a function that returns true if a string of brackets is valid/balanced](https://repl.it/@bgando/bracket-validator-prompt)
  - [Solution](https://replit.com/@bgando/bracket-validator-solution)

#### Linked Lists - Prompts and Solutions

- Delete the following:
  - [a node in the middle of the linked list](https://repl.it/@bgando/ll-delete-mid-node-prompt)
    - [Solution](https://replit.com/@bgando/ll-delete-mid-node-solution)
  - [a node with only a variable pointing at that node](https://repl.it/@bgando/ll-delete-node-no-pointers-prompt)
    - [Solution](https://replit.com/@bgando/ll-delete-node-no-pointers-solution)
  - [a duplicate](https://repl.it/@bgando/ll-delete-dupe-node-prompt)
    - [Solution](https://replit.com/@bgando/ll-delete-dupe-node-solution)
- [Partition a linked list around a value.](https://repl.it/@bgando/ll-partition-prompt)
  - [Solution](https://replit.com/@bgando/ll-partition-solution)
- [Write a function for reversing a linked list.](https://repl.it/@bgando/ll-reverse-prompt)
  - [Solution](https://replit.com/@bgando/ll-reverse-solution)
- [Can you do it in-place?](https://repl.it/@bgando/ll-reverse-prompt)
- [Check if a linked list contains a cycle.](https://repl.it/@bgando/ll-cycle-detect-prompt)
  - [Solution](https://replit.com/@bgando/ll-cycle-detect-solution)
- [Find the kth to the last node.](https://repl.it/@bgando/ll-kth-to-last-node-prompt)
  - [Solution](https://replit.com/@bgando/ll-kth-to-last-node-solution)

#### Hash Tables - Prompts and Solutions

- [Count the number of occurrences of all characters or words in a body of text or string.](https://repl.it/@bgando/ht-unique-string-prompt)
  - [Solution](https://replit.com/@bgando/ht-unique-string-solution)
- [Delete duplicates in a list.](https://repl.it/@bgando/ht-remove-dups-prompt)
  - [Solution](https://replit.com/@bgando/ht-remove-dups-solution)
- [Find a unique value in a list.](https://repl.it/@bgando/ht-find-unique-list-prompt)
  - [Solution](https://replit.com/@bgando/ht-find-unique-list-solution)
- [Find if two integers in a list add up to k](https://repl.it/@bgando/ht-two-items-sum)
  - [Solution](https://replit.com/@bgando/ht-two-items-sum-solution)

#### Arrays and Strings - Prompts and Solutions

- [Rotate a matrix, string, or an array](https://repl.it/@bgando/rotate-array-prompt)
  - [Solution](https://replit.com/@bgando/rotate-array-solution)
- [Given an m x n boolean matrix, if an element is 0, set its entire row and column to 0.](https://repl.it/@bgando/boolean-matrix-prompt)
  - [Solution](https://replit.com/@bgando/boolean-matrix-solution)
- [Search for a value.](https://repl.it/@bgando/search-for-value-prompt)
  - [Solution](https://replit.com/@bgando/search-for-value-solution)
- [Write a function encodes a string by turning all spaces into \`%20\`.](https://repl.it/@bgando/encode-string-prompt)
  - [Solution](https://replit.com/@bgando/encode-string-solution)
- [Merge two sorted lists into one list.](https://repl.it/@bgando/merge-sorted-arrays-prompt)
